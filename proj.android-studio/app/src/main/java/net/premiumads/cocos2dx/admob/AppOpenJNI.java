...
public final class AppOpenJNI {
    private static AppOpenAd sAd;

    public static native void nativeOnEvent(int code, String msg);

    public static void load(final String adUnitId, final int orientation) {
        final Activity act = (Activity) Cocos2dxActivity.getContext();
        act.runOnUiThread(new Runnable() { @Override public void run() {
            AppOpenAd.load(act, adUnitId, new AdRequest.Builder().build(), orientation,
