#include "RewardedInterstitial.h"
#include "AdMobBridgeCpp.h"
#include "platform/CCPlatformConfig.h"

#if (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
    #include "platform/android/jni/JniHelper.h"
    static const char* kJNI = "net/premiumads/cocos2dx/admob/RewardedInterstitialJNI";
#elif (CC_TARGET_PLATFORM == CC_PLATFORM_IOS)
    extern "C" void admob_ios_rewardedInterstitial_load(const char* adUnitId);
    extern "C" bool admob_ios_rewardedInterstitial_isReady();
    extern "C" void admob_ios_rewardedInterstitial_show();
#endif

namespace admob {

RewardedInterstitial::Listener RewardedInterstitial::s_listener;
void RewardedInterstitial::setListener(Listener cb) { s_listener = std::move(cb); }

void RewardedInterstitial::load(const std::string& adUnitId) {
#if (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
...
#elif (CC_TARGET_PLATFORM == CC_PLATFORM_IOS)
    admob_ios_rewardedInterstitial_load(adUnitId.c_str());
#endif
}

bool RewardedInterstitial::isReady() {
#if (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
    cocos2d::JniMethodInfo info;
    ...
    return false;
#elif (CC_TARGET_PLATFORM == CC_PLATFORM_IOS)
    return admob_ios_rewardedInterstitial_isReady();
#else
    return false;
#endif
}

void RewardedInterstitial::show() {
#if (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
    cocos2d::JniMethodInfo info;
...
#elif (CC_TARGET_PLATFORM == CC_PLATFORM_IOS)
    admob_ios_rewardedInterstitial_show();
#endif
}

void RewardedInterstitial::onPlatformEvent(int code, const std::string& msg) {
    if (!s_listener) return;
    auto e = static_cast<RewardedInterstitialEvent>(code);
...
Java_net_premiumads_cocos2dx_admob_RewardedInterstitialJNI_nativeOnEvent(
        JNIEnv* env, jclass, jint code, jstring msg) {
    const char* c = msg ? env->GetStringUTFChars(msg, nullptr) : "";
    admob::RewardedInterstitial::onPlatformEvent((int)code, std::string(c ? c : ""));
